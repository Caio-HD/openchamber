...
Tu página puede saber en cuál de los dos está gracias a `ctx.surface` (`panel` o `dialog`). Cuando el usuario hace clic en un elemento que adjuntaste antes, la misma superficie se abre de nuevo con `ctx.item` apuntando a ese elemento, incluidos los `data` que guardaste en él, así que puedes mostrar sus detalles en lugar de la lista. Desde el menú + o el icono de la barra lateral, `ctx.item` es `null`.
...
`contributes.service` es opcional. Declara un servicio local (un proceso que OpenChamber inicia junto a la extensión) para cosas a las que una página web no puede llegar, como un socket de Docker. Consulta el archivo `GUEST_SERVICES.md` del paquete.
...
| `sessions` | crear sesiones y worktrees (`startSession`) |
