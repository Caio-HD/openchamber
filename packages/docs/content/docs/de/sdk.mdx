...
Deine Seite erkennt über `ctx.surface` (`panel` oder `dialog`), wo sie gerade läuft. Wenn der Nutzer einen Eintrag anklickt, den du früher angehängt hast, öffnet sich dieselbe Oberfläche erneut mit `ctx.item` auf diesem Eintrag, einschließlich aller `data`, die du darauf gespeichert hast, sodass du seine Details statt der Liste zeigen kannst. Aus dem +-Menü oder über das Symbol in der Leiste ist `ctx.item` `null`.
