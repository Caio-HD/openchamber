...
Sayfanız `ctx.surface` (`panel` veya `dialog`) üzerinden hangisinde olduğunu anlayabilir. Kullanıcı daha önce iliştirdiğiniz bir öğeye tıkladığında aynı yüzey yeniden açılır ve `ctx.item` o öğeye, üzerine kaydettiğiniz `data` dahil, ayarlanır; böylece liste yerine öğenin ayrıntılarını gösterebilirsiniz. + menüsünden veya şerit simgesinden açıldığında `ctx.item` `null` olur.
...
`contributes.service` isteğe bağlıdır. OpenChamber'ın eklentinin yanında başlattığı yerel bir süreç tanımlar; bir web sayfasının ulaşamadığı şeyler için, örneğin bir Docker soketi. Paket dosyası `GUEST_SERVICES.md` dosyasına bakın.
...
| `sessions` | oturum ve worktree oluşturmak (`startSession`) |
