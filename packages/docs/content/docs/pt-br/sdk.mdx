...
Sua página consegue saber em qual dos dois está por `ctx.surface` (`panel` ou `dialog`). Quando o usuário clica em um item que você anexou antes, a mesma superfície abre de novo com `ctx.item` apontando para esse item, incluindo qualquer `data` que você guardou nele, então você pode mostrar os detalhes dele em vez da lista. A partir do menu + ou do ícone da barra lateral, `ctx.item` é `null`.
...
`contributes.service` é opcional. Ele declara um serviço local (um processo que o OpenChamber inicia junto com a extensão) para coisas que uma página web não alcança, como um socket do Docker. Veja o arquivo `GUEST_SERVICES.md` do pacote.
...
| `sessions` | criar sessões e worktrees (`startSession`) |
