...
Votre page sait dans lequel des deux elle se trouve grâce à `ctx.surface` (`panel` ou `dialog`). Quand l'utilisateur clique sur un élément que vous avez attaché plus tôt, la même surface s'ouvre à nouveau avec `ctx.item` pointant sur cet élément, y compris les `data` que vous y avez stockées, vous pouvez donc afficher ses détails au lieu de la liste. Depuis le menu + ou l'icône de la barre latérale, `ctx.item` vaut `null`.
