...
Twoja strona rozpozna, gdzie się znajduje, po `ctx.surface` (`panel` lub `dialog`). Gdy użytkownik kliknie element, który wcześniej dołączyłeś, ta sama powierzchnia otwiera się ponownie z `ctx.item` ustawionym na ten element, razem z `data`, które na nim zapisałeś, więc możesz pokazać jego szczegóły zamiast listy. Z menu + lub z ikony na pasku bocznym `ctx.item` to `null`.
